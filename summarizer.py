...
async def summarize_turn_background(
    ...
    Returns True if at least one summary was written, False otherwise.
...
_STALE_THRESHOLD = 100  # summaries longer than this were generated by old prompts


async def resummarize_session(
    config: "Config",
    session_id: str,
    force: bool = False,
) -> tuple[int, int]:
    """Re-summarize Q/A entries whose summaries are missing or stale (too long).

    Returns (updated, skipped) counts.
    force=True re-summarizes all entries regardless of length.
    """
    from agent.memory.qa_log import QALogger
    import asyncio as _asyncio

    logger_obj = QALogger(session_id)
    q_dir = logger_obj._get_q_dir()
    a_dir = logger_obj._get_a_dir()

    if not q_dir.exists() and not a_dir.exists():
        return 0, 0

    def _needs_resummarize(summary: str) -> bool:
        if not summary:
            return True
        return force or len(summary) > _STALE_THRESHOLD

    # Collect file pairs by turn_id
    pairs: dict[int, dict] = {}
    for q_file in sorted(q_dir.glob("Q-*.json")) if q_dir.exists() else []:
        try:
            data = json.loads(q_file.read_text(encoding="utf-8"))
            tid = data.get("turn_id")
            if tid is not None:
                pairs.setdefault(tid, {})["q_path"] = q_file
                pairs[tid]["q_data"] = data
        except Exception:
            continue
    for a_file in sorted(a_dir.glob("A-*.json")) if a_dir.exists() else []:
        try:
            data = json.loads(a_file.read_text(encoding="utf-8"))
            tid = data.get("turn_id")
            if tid is not None:
                pairs.setdefault(tid, {})["a_path"] = a_file
                pairs[tid]["a_data"] = data
        except Exception:
            continue
    ...
    for tid in sorted(pairs):
        entry = pairs[tid]
        q_data = entry.get("q_data", {})
        a_data = entry.get("a_data", {})
        q_path = entry.get("q_path")
        a_path = entry.get("a_path")

        q_needs = q_path and _needs_resummarize(q_data.get("summary_q", ""))
        a_needs = a_path and _needs_resummarize(a_data.get("summary_a", ""))

        if not q_needs and not a_needs:
            skipped += 1
            continue

        try:
            q_content = q_data.get("content", "") if q_needs else ""
            a_content = a_data.get("content", "") if a_needs else ""

            summary_q, summary_a = await _asyncio.gather(
                _call_llm_one_line(config, _Q_SYSTEM, q_content) if q_content else _asyncio.sleep(0, result=""),
                _call_llm_one_line(config, _A_SYSTEM, a_content) if a_content else _asyncio.sleep(0, result=""),
            )

            if summary_q and q_path:
                await _asyncio.to_thread(_update_json_file, q_path, {"summary_q": summary_q})
            if summary_a and a_path:
                await _asyncio.to_thread(_update_json_file, a_path, {"summary_a": summary_a})
            updated += 1
        except _asyncio.CancelledError:
            raise
        except Exception as exc:
            logger.warning("resummarize_session: turn %s failed (skipped): %s", tid, exc)
            skipped += 1
