...
def _apply_unified_diff(original: str, patch: str) -> str:
    # -u: unified only. Without it GNU patch also accepts ed-style scripts,
    # which it pipes to ed(1) on the host.
    import subprocess
    ...
    try:
        try:
            result = subprocess.run(
                ["patch", "-u", "--dry-run", orig_path, patch_path],
