...
@register("search_archive", {
    ...
    "parameters": {
        "type": "object",
        "properties": {
            "query": {"type": "string", "description": "Keyword search query (FTS)"},
            "top_k": {"type": "integer", "description": "Number of results to return (default: 8)"},
        },
        "required": ["query"],
    },
...
def search_archive(query: str, top_k: int | None = None) -> dict:
    ...
    for r in rows:
        ...
        cleaned.append({
            ...
            "archived_at": _dt.datetime.fromtimestamp(archived_at).isoformat(timespec="seconds") if archived_at else None,
            "reason": r.get("reason"),
        })
    return {"results": cleaned, "count": len(cleaned), "query": query, "source": "archive"}
